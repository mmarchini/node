...
// generate a proper test file. The comment below is replaced by
// `tools/gen-postmortem-metadata.py` to add the constants generated by it.
...
TEST_F(DebugSymbolsTest, ContextEmbedderDataIndex) {
  EXPECT_EQ(nodedbg_environment_context_idx_embedder_data,
...
TEST_F(DebugSymbolsTest, BaseObjectPersistentHandle) {
  ...
  auto calculated =
      (uintptr_t)(obj) + nodedbg_class__BaseObject__persistent_handle;
...
TEST_F(DebugSymbolsTest, EnvironmentHandleWrapQueue) {
  ...
  auto calculated =
      ((uintptr_t)(*env)) + nodedbg_class__Environment__handleWrapQueue;
  EXPECT_EQ(expected, calculated);
}
...
TEST_F(DebugSymbolsTest, EnvironmentReqWrapQueue) {
  ...
  auto calculated =
      ((uintptr_t)(*env)) + nodedbg_class__Environment__reqWrapQueue;
  EXPECT_EQ(expected, calculated);
}
