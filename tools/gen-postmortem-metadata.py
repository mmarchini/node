...
def render_symbols_file():
  ...
  std_includes = get_standard_includes()
  std_includes = ['#include <{0}>'.format(include) for include in std_includes]
  std_includes = '\n'.join(std_includes)
  ...
  return template.format(
    filename=sys.argv[0],
    includes=includes,
    standard_includes=std_includes,
    declare_symbols=declare_symbols,
    fill_symbols=fill_symbols,
  )


def create_symbols_file(symbols_file):
  out = file(sys.argv[1], 'w')
  out.write(render_symbols_file())


def create_test_file(symbols_file):
  test_template = file('test/cctest/test_node_postmortem_metadata.cc').read()
  out = file(sys.argv[2], 'w')
  test_file_replace_string = '// {{debug_symbols_file}}'
  out.write(test_template.replace(test_file_replace_string, symbols_file))


rendered_symbols_file = render_symbols_file()
create_symbols_file(rendered_symbols_file)
create_test_file(rendered_symbols_file)
